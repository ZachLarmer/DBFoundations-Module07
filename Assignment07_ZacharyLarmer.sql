...
-- 11-25-2024, ZacharyLarmer, Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_ZacharyLarmer')
	 Begin 
	  Alter Database [Assignment07DB_ZacharyLarmer] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_ZacharyLarmer;
	 End
	Create Database Assignment07DB_ZacharyLarmer;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_ZacharyLarmer;
...
SELECT ProductName, FORMAT(UnitPrice, 'C', 'en-US') FROM Products;
*/
...
SELECT 
	...
	InventoryDate, 
	[Count],
	ISNULL(CAST(LAG([Count]) OVER (PARTITION BY ProductName ORDER BY ProductName) AS INT), 0) AS PreviousMonthCount
